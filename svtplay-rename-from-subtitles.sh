...
if [[ ! -f title-suggestions.txt ]]; then
  ...
  sed -e 's/\(.*\.E[0-9][0-9]*\.\)\(.*\)\.srt-\(.*\)$/mv "\1\2.mp4" "\1\3.mp4"/' title-suggestions.txt > title-script.sh
  sed -e 's/\(.*\.E[0-9][0-9]*\.\)\(.*\)\.srt-\(.*\)$/mv "\1\2.srt" "\1\3.srt"/' title-suggestions.txt >> title-script.sh
